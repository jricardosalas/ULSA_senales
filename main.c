    ...
    if(buf_uart[len_uart-1] == '\r'){ //  \n end of line character
        ...
        if(buf_uart[0] == 'M'){ // Utility commands
            ...
            if(buf_uart[1]=='1'){ //Hardware
                ...
                // dataSensor = readADC(ADC_channelSensor1); 
                dataSensor = -30; // dummy for testing
                sprintf(str, "%d" ,dataSensor);
                putchsUART("Sensor 1: ");
                putchsUART(str);
                putchsUART("\r\n");
            }else if(buf_uart[1]=='2'){
                // dataSensor = readADC(ADC_channelSensor2);
                dataSensor = 105;
                sprintf(str, "%d" ,dataSensor);
                putchsUART("Sensor 2: ");
                putchsUART(str);
                ...
                // dataSensor = readADC(ADC_channelSensor3);
                dataSensor = 15;
                sprintf(str, "%d" ,dataSensor);
                putchsUART("Sensor 3: ");
                putchsUART(str);
